...
export default function Statistics() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/random_metrics_continuous`);
        const data = await res.json();

        setStats((prev) => ({
          users: Math.round(prev.users + (data.users - prev.users) * 0.2),
          uptime_percent: Math.round(prev.uptime_percent + (data.uptime_percent - prev.uptime_percent) * 0.2),
          tasks_completed: Math.round(prev.tasks_completed + (data.tasks_completed - prev.tasks_completed) * 0.2),
          revenue: Math.round(prev.revenue + (data.revenue - prev.revenue) * 0.2),
        }));
      } catch (error) {
